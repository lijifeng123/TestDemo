...
- (void)setAutoLoadNextPage:(BOOL)autoLoadNextPage
{
    self.infiniteScrollingView.autoLoadNextPage = autoLoadNextPage;
}

- (BOOL)isAutoLoadNextPage
{
    return self.infiniteScrollingView.isAutoLoadNextPage;
}
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.enabled = YES;
        self.autoLoadNextPage = YES;
        
        UILabel * nomalLable = [[[UILabel alloc] init] autorelease];
        [nomalLable setFrame:CGRectMake(CGRectGetMaxX(self.activityIndicatorView.frame), 0, 100, frame.size.height)];
        [nomalLable setText:@"上拉显示更多"];
        [nomalLable setTextAlignment:NSTextAlignmentCenter];
        [nomalLable setBackgroundColor:[UIColor clearColor]];
        [nomalLable setTextColor:[SVInfiniteScrollingView colorWithHexString:@"666666"]];
        [nomalLable setFont:[UIFont systemFontOfSize:14]];
        
        UILabel * loadLabel = [[[UILabel alloc] init] autorelease];
        [loadLabel setFrame:CGRectMake(CGRectGetMaxX(self.activityIndicatorView.frame), 0, 100, frame.size.height)];
        [loadLabel setText:@"正在加载..."];
        [loadLabel setTextAlignment:NSTextAlignmentCenter];
        [loadLabel setBackgroundColor:[UIColor clearColor]];
        [loadLabel setTextColor:[SVInfiniteScrollingView colorWithHexString:@"666666"]];
        [loadLabel setFont:[UIFont systemFontOfSize:14]];
        
        self.viewForState = [NSMutableArray arrayWithObjects:@"",nomalLable,loadLabel,@"", nil];
    }
...
- (void)layoutSubviews {
    ...
    self.activityIndicatorView.center = CGPointMake(self.bounds.size.width/4, self.bounds.size.height/2);
}
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVInfiniteScrollingStateLoading && self.enabled) {
        ...
        if (self.isAutoLoadNextPage) {
            CGFloat scrollOffsetThreshold = scrollViewContentHeight-(self.scrollView.bounds.size.height*2);
            
            if(self.state == SVInfiniteScrollingStateTriggered) {
                if (self.scrollView.contentOffset.y > 0) {
                    self.state = SVInfiniteScrollingStateLoading;
                }
            } else if(contentOffset.y > 0 && contentOffset.y > scrollOffsetThreshold && self.state == SVInfiniteScrollingStateStopped) {
...
//    else {    //Modify by zhuhao at 20131010
...
//              [self.activityIndicatorView startAnimating]; //Modify by zhuhao at 20131010
                break;
